...
const Header = () => {
    ...
    return(
      <>
        ...
        {!info.loading && info.data.siteName ? (
          <div id="logo" className={`text-[2.1rem] maxmob:text-[1.85rem] smmob:text-[1.6rem] tracking-[-0.5px] font-semibold inline-block whitespace-nowrap z-[1]`}>
            <Link to={"/"} className="no-underline text-gainsboro-100 font-['able']">
              {info.data.siteName}
            </Link>
          </div>
        ):(null)}
  ...
  return (
    <header id="home" dir="ltr" className={`font-['able'] h-[4rem] maxmob:h-[3.5rem] transition-all z-50 bg-darkslategray [backdrop-filter:blur(20px)] w-full text-gainsboro-100 flex fixed top-0`}>
