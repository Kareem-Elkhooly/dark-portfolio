...
const LoginFeilds = () => {
    ...
    const submit = (e) => {
        ...
        if(!emailValidate){
            ...
            .catch(err => setMessage(messageError("invalid email or password")));
        }
    }
      ...
      <form id="loginForm" onSubmit={submit} className="flex flex-col items-start justify-center gap-[1.3rem] w-[30rem] max-w-full border-darkslategray border-solid rounded-md">
        ...
        <div className="flex flex-col items-start justify-center gap-[1.5rem] w-full">
            ...
            <div className="flex flex-col items-start gap-1 w-full">
                ...
                <input type="password" name="password" id="password" className="text-gainsboro-100 bg-darkslategray w-full p-3 text-[1rem] h-[2.4rem] border-2 border-transparent border-solid rounded focus-visible:border-zinc-700 focus-visible:outline-none"></input>
            </div>
            ...
            <div className="flex items-end justify-end w-full mt-6">
                <button type="submit" value="login" className="text-[1.1rem] px-8 py-2 rounded cursor-pointer font-semibold bg-gainsboro-200 hover:bg-darkslategray text-darkslategray hover:text-gainsboro-200 transition-all duration-200 ease-in">Login</button>
            </div>
        </div>
      </form>
    </section>
