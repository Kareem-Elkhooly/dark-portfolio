...
const DashSide = () => {
  ...
  const link = (title, icon, to, textEdit)=>{
    return (
        <Link to={"/admin/dashboard/"+to} className={`${textEdit || "text-gainsboro-200"} flex gap-2 w-full items-center no-underline capitalize whitespace-nowrap hover:text-gainsboro-100 text-[1.2rem] maxmob:text-[1.4rem] maxmob:font-semibold transition-all duration-100 ease-in`}>
  ...
  return (
    <section className={`${enFontFamily} w-[16rem] maxmob:w-fit pl-6 smmob:pl-[0.8rem] pt-[6rem] pb-5 pr-4 smmob:pr-[0.8rem] bg-gray overflow-auto h-[calc(100vh)] z-0`}>
      <div className="flex justify-between smmob:justify-start flex-col h-full items-start w-full ">
        ...
        <div>
          <a href="/admin" onClick={logOut} className="text-[1rem] smmob:mt-10 maxmob:text-[1.1rem] flex items-center justify-center gap-1 no-underline text-whitesmoke bg-red-600 font-medium hover:bg-red-500 transition-all duration-100 ease-in rounded-md px-3 maxmob:px-1 py-[4px] maxmob:py-3">
