...
export default function SectionBody({state, stateName , item, children}) {
  return (
    <div className="flex flex-col max-h-[30rem] overflow-auto font-semibold gap-8 px-6 py-8 maxmob:px-2 maxmob:py-4 w-full mt-4 border-2 border-solid border-gainsboro-200 rounded-sm">
      {state.loading ? (
      ...
      {!state.loading && (item || state.data)?(
        ...
        <div>
          <p className="text-[1.3rem] text-darkslategray text-center">No {stateName} yet.</p>
        </div>
      ):(null)}
    </div>
  )
