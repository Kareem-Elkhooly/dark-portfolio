...
import { faCaretUp, faCaretDown, faCaretRight, faCaretLeft } from "@fortawesome/free-solid-svg-icons";
...
const Work = () => {
  ...
  const projectShow = (project, language, titleEdit, textEdit, dir)=>{
    return (
       <div className={`${dir} w-full min-h-screen flex items-center maxmob:items-start justify-center maxmob:flex-wrap maxmob:flex-col gap-[2.125rem] maxmob:gap-[3rem]`}>
        ...
        <div className="relative flex justify-center proj h-[47.688rem] w-[722px] maxlg:h-[40.688rem] maxlg:max-w-[600px] maxmob:max-w-full maxmob:max-h-[30rem] maxmob:self-center bg-darkslategray rounded-md">
          <FontAwesomeIcon onClick={function(){nextImage(project)}} id={"nextOf_"+project._id} icon={faCaretRight}
            className="absolute z-10 text-[1.3rem] text-darkslategray bg-gainsboro-100 hover:bg-zinc-300 rounded px-2 py-4 shadow-lg -translate-y-1/2 top-[50%] right-[-12px] maxmob:right-[8px] cursor-pointer" 
          />
          <img
            id={"img_" + project._id}
          ...
          <FontAwesomeIcon onClick={function(){prevImage(project)}} id={"prevOf_"+project._id} icon={faCaretLeft}
            className="hidden z-10 absolute text-[1.3rem] text-darkslategray bg-gainsboro-100 hover:bg-zinc-300 rounded px-2 py-4 shadow-lg -translate-y-1/2 top-[50%] left-[-12px] maxmob:left-[8px] cursor-pointer" 
          />
        </div>
      </div>
    )
  ...
  return (
    <div className="w-full relative bg-gray overflow-hidden tracking-[normal] flex flex-col gap-[3rem] smmob:gap-[1rem]">
      <Header />
          <div id="slideBtnsWork" className="fixed bottom-[2rem] right-6 z-50 text-zinc-300 hidden flex-col gap-4 text-[1.3rem] smmob:flex-row transtion-all duration-100 ease-in">
            <FontAwesomeIcon id="slideBtnWorkUp" className="cursor-pointer text-darkslategray bg-gainsboro-100 hover:bg-zinc-300 rounded px-5 py-1" icon={faCaretUp} />
            <FontAwesomeIcon id="slideBtnWorkDown" className="cursor-pointer text-darkslategray bg-gainsboro-100 hover:bg-zinc-300 rounded px-5 py-1" icon={faCaretDown} />
          </div>
      <Informations />
